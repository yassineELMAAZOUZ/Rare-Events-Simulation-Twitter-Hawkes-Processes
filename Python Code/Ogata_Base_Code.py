...
@jit(nopython = True)
def intensityCalculator(params,jumpTimes):
    """  
		Input : params  = (T,lambda0,alpha,beta)
...
@jit(nopython = True)
def logLikelihood(params,jumpTimes):
    """  
		Input : params  = (T,lambda0,alpha,beta)
    ...
    intensityValues = intensityCalculator(params,jumpTimes)
...
@jit(nopython = True)
def likeliHood(params1,params2,jumpTimes):
    """  
		Input : params  = (T,lambda0,alpha,beta)
    ...
    logdP1_P0 = logLikelihood(params1,jumpTimes)
    logdP2_P0 = logLikelihood(params2,jumpTimes)

    return   np.exp(logdP1_P0-logdP2_P0)


@jit(nopython = True)      
def asymptoticExpextedValue(params):
    """  
		Input : params  = (T,lambda0,alpha,beta)
...
@jit(nopython = True)
def getLambda0(T,alpha,beta,expectedValue):
    """
...
def asymptoticStd(params):
    ...
    return  np.sqrt((lambda0*T) / ((1 - (alpha/beta))**3))
